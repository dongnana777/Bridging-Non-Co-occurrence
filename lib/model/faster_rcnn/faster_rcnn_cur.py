...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, open):
        ...
        if self.training:
            ...
            rois, rois_label, rois_target, rois_inside_ws, rois_outside_ws, rois_selected = roi_data
        ...
        if self.training and num_boxes>0:
            ...
            rois_selected = Variable(rois_selected)
        else:
        ...
        if cfg.POOLING_MODE == 'align':
            ...
            if self.training and num_boxes>0:
                pooled_feat_positive_selected = self.RCNN_roi_align(base_feat, rois_selected.view(-1, 5))
        ...
        # compute bbox offset
        bbox_pred_ = self.RCNN_bbox_pred(pooled_feat)  # [256,84]

        if self.training and not self.class_agnostic:
            # select the corresponding columns according to roi labels
            bbox_pred_view = bbox_pred_.view(bbox_pred_.size(0), int(bbox_pred_.size(1) / 4), 4)
        ...
        if self.training:
            return rois, cls_score, bbox_pred_, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox,\
                   rois_label, base_feat, pooled_feat_positive_selected, rpn_cls_score, rpn_bbox_pred
        else:
