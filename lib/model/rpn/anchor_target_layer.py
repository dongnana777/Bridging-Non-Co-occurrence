...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(batch_size):
            ...
            mask_per_im_near = torch.zeros([height, width], dtype=torch.int64).cuda()
            for k in range(gt_boxes.shape[1]):
                if torch.sum(gt_boxes[i][k]) == 0.:
                    break
                max_iou_per_gt = max_iou[k] * 0.9
                min_iou_per_gt = max_iou[k] * 0.5
                mask_per_gt_near = torch.sum(IOU_map[i][:, :, :, k] > min_iou_per_gt, dim=2)
                mask_per_gt = torch.sum(IOU_map[i][:, :, :, k] > max_iou_per_gt, dim=2)
                mask_per_im_near += mask_per_gt_near
                mask_per_im += mask_per_gt
            mask_batch.append(mask_per_im_near)
